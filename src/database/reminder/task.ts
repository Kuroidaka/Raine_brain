...
export class TaskService {
  ...
  private mapAreaToPrisma(
    area?: Areas[]
  ): Prisma.TaskAreasCreateNestedManyWithoutTaskInput | undefined {
  ...
  async getTasksByDateRange(userId: string, startDate: string | Date, endDate: string | Date) {
    return await dbClient.task.findMany({
      where: {
        userId: userId,
        deadline: {
          gte: new Date(startDate),  // greater than or equal to the startDate
          lte: new Date(endDate),    // less than or equal to the endDate
        },
      },
      include: {
        area: true,           // Include TaskAreas
        subTask: true,        // Include SubTasks
        taskAttachment: true, // Include TaskAttachments
      },
      orderBy: {
        createdAt: 'desc'
      }
  ...
  async getSubTask(taskID: string) {
    try {
      return await dbClient.subTask.findMany({
        where: {
          taskId: taskID,
        },
      });
  ...
  async addTaskAttachment(taskId: string, data: TaskAttachmentProps) {
    try {
      return await dbClient.taskAttachment.create({
        data: {
          ...data,
          taskId,
        },

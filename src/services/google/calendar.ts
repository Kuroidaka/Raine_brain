...
export const GoogleService = {
  createEvent: async (
    ...
    try {
      ...
      throw new InternalServerErrorException(
        "error occur while process creating google calendar"
      );
    }
  },
  getCalendarList: async () => {
    try {
      ...
      console.error(error);
      throw new InternalServerErrorException(
        "Error occurred while retrieving the calendar list from Google Calendar"
      );
    }
  },
  getEventList: async () => {
  ...
  getEvent: async (id: string, eventListId: string) => {
    try {
      const calendar = google.calendar({
        version: "v3",
        auth: googleOAuth2Client,
  ...
  deleteCalendarEvent: async (eventId: string, eventListId: string) => {
    try {
      ...
      console.error(error);
      throw new InternalServerErrorException(
        "Error occurred while processing the deletion of a Google Calendar event"
      );
    }
  },
  ...
  createTask: async (
    eventListId: string,
    { note, status, title, due }: taskCreate
  ) => {
    try {
      const task = google.tasks({ version: "v1", auth: googleOAuth2Client });
  ...
  getAllEvent: async (eventListId: string) => {
    try {
      const calendar = google.calendar({
        version: "v3",
        auth: googleOAuth2Client,
